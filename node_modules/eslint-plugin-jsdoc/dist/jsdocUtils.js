...
'implements', // "typeName"
'type']);
const tagsWithMandatoryTypePositionTypeScript = new Set([...tagsWithMandatoryTypePosition, 'this']);
const tagsWithMandatoryTypePositionClosure = new Set([...tagsWithMandatoryTypePositionTypeScript, 'define']); // All of these have a signature with "type" except for
...
'modifies']);
const tagsWithOptionalTypePositionTypescript = new Set(['template', ...tagsWithOptionalTypePosition]);
const tagsWithOptionalTypePositionClosure = new Set([...tagsWithOptionalTypePositionTypescript, 'export', // Shows the signature with curly brackets but not in the example
// "typeExpression"
...
const closureNamepathDefiningTags = new Set([// Though defines in a sense, it is not parseable in the same way
...
'alias', 'augments', 'extends', 'lends', // Signature seems to require a "namepath" (and no counter-examples),
...
const tagsWithOptionalNamePosition = new Set([// Signature seems to require a "namepath" (and no counter-examples)
// Not used with namepath in Closure/TypeScript, however
'this', ...namepathDefiningTags, ...tagsWithOptionalNamePositionBase]);
const typescriptTagsWithOptionalNamePosition = new Set(['template', ...typescriptNamepathDefiningTags, ...tagsWithOptionalNamePositionBase]);
const closureTagsWithOptionalNamePosition = new Set(['template', ...closureNamepathDefiningTags, ...tagsWithOptionalNamePositionBase]); // Todo: `@link` seems to require a namepath OR URL and might be checked as such.
...
const tagsWithMandatoryTypeOrNamePositionBase = new Set([// "namepath"
'alias', 'augments', 'extends', 'borrows', 'lends', 'memberof', 'memberof!', 'name', 'typedef', 'external', 'host', // "OtherObjectPath"
'mixes']);
const tagsWithMandatoryTypeOrNamePosition = new Set([// namepath
'this', ...tagsWithMandatoryTypeOrNamePositionBase]);
const tagsWithMandatoryTypeOrNamePositionTypescript = new Set([...tagsWithMandatoryTypeOrNamePositionBase]);
...
const tagMightHaveTypePosition = (mode, tag) => {
  ...
  if (mode === 'typescript') {
    return tagsWithMandatoryTypePositionTypeScript.has(tag) || tagsWithOptionalTypePositionTypescript.has(tag);
  }
